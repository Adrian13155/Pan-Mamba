...
import numbers
from Vim.mamba.mamba_ssm.modules.mamba_simple import Mamba
from pansharpening.model.refine import Refine
def to_3d(x):
...
if __name__ == '__main__':
    ...
    ms = torch.rand([1,4,32,32]).cuda()
    pan = torch.rand([1,1,128,128]).cuda()
    gt = torch.rand([1,4,128,128]).cuda()
    net = Net(num_channels=4, base_filter=32).cuda()
    x = net(ms,None,pan)
    sharpen_result = x

import sys
sys.path.append("/PublicData/xmm/project/Pan-Mamba")
import os
...
import logging 
from datetime import datetime
from pansharpening.model.panmamba_baseline_finalversion import Net
...
def main(opt):
    ...
    if opt.satellite == "WV2":
        ...
        channel = 4
    model = Net(channel, base_filter=32).cuda()
    ...
    optimizer_G = torch.optim.Adam(model.parameters(), lr=lr, betas=(0.9,0.999), eps=1e-8)
    lr_scheduler_G = CosineAnnealingWarmRestarts(optimizer_G,500,1,5e-8)
    ...
    for epoch in range(opt.epoch_start,num_epoch):
        ...
        for index, data in enumerate(pbar):
            ...
            restored  = model(inp_ms ,None,inp_pan)
        ...
        if epoch % 10== 0:
            ...
            with torch.no_grad():
                ...
                for index, datas in enumerate(tqdm(val_dataloader)):
                    ...
                    output = model(inp_ms ,None,inp_pan)
...
def get_opt():
    ...
    parser.add_argument('--exp_name', type=str, default='PanMamba', help='experiment name')
    ...
    parser.add_argument('-save_dir', help='日志保存路径', default='/PublicData/xmm/project/Pan-Mamba/experiment', type=str)
